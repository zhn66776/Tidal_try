...
# 分割数据集
train_size = int(len(dataset) * 0.833)  # 60% 训练
val_size = int(len(dataset) * 0.08)    # 20% 验证
test_size = len(dataset) - train_size - val_size  # 20% 测试
...
learning_rate = 0.003  # 设置学习率
weight_decay = 0.0001    # 设置 L2 正则化系数

model = Sequential()
model.add(LSTM(25, input_shape=(1, look_back), kernel_regularizer=l2(weight_decay)))
...
history = model.fit(trainX, trainY, validation_data=(valX, valY), epochs=300, batch_size=240, verbose=1)
...
# 计算滚动预测的 RMSE
testScore = math.sqrt(mean_squared_error(test_actual_data, rolling_predictions))
